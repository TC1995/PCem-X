...
static uint32_t rivatnt_pmc_read(uint32_t addr, void *p)
{
  ...
  switch(addr)
  {
  case 0x000000: ret = 0x00030111; break;
  }

  return ret;
}

static uint32_t rivatnt_pbus_read(uint32_t addr, void *p)
{
  rivatnt_t *rivatnt = (rivatnt_t *)p;
  svga_t *svga = &rivatnt->svga;
  uint32_t ret = 0;
  ...
  case 0x001800 ... 0x0018ff:
    ret = rivatnt_pci_read(0, addr - 0x1800, rivatnt);
    ret |= rivatnt_pci_read(0, addr + 1 - 0x1800, rivatnt) << 8;
    ret |= rivatnt_pci_read(0, addr + 2 - 0x1800, rivatnt) << 16;
    ret |= rivatnt_pci_read(0, addr + 3 - 0x1800, rivatnt) << 24;
    break;
  }
...
static uint32_t rivatnt_pfb_read(uint32_t addr, void *p)
{
  rivatnt_t *rivatnt = (rivatnt_t *)p;
  svga_t *svga = &rivatnt->svga;
  uint32_t ret = 0;
  ...
  case 0x100200: ret = rivatnt->pfb.config_0; break;
  }
...
static uint8_t rivatnt_pramdac_read(uint32_t addr, void *p)
{
  ...
  switch(addr)
  {
  case 0x680600: ret = rivatnt->pramdac.gen_ctrl; break;
  }
...
static uint8_t rivatnt_mmio_read(uint32_t addr, void *p)
{
  ...
  ret = rivatnt_mmio_read_l(addr, rivatnt);
  ret >>= ((addr & 3)) << 3;
  ret &= 0xff;
}
...
static void rivatnt_mmio_write(uint32_t addr, uint8_t val, void *p)
{
  ...
  tmp = rivatnt_mmio_read_l(addr, rivatnt);
  tmp &= ~(0xff << ((addr & 3) << 3));
  tmp |= val << ((addr & 3) << 3);
  rivatnt_mmio_write_l(addr, tmp, rivatnt);
}
...
static void rivatnt_rma_out(uint16_t addr, uint32_t val, void *p)
{
  rivatnt_t *rivatnt = (rivatnt_t *)p;
  svga_t *svga = &rivatnt->svga;
...
static void rivatnt_out(uint16_t addr, uint8_t val, void *p)
{
  ...
  if(addr == 0x3d3) rivatnt_rma_out(rivatnt->rma_addr + ((rivatnt->rma.mode & 0xe) << 1), *(uint32_t*)rivatnt->rma.access_reg, rivatnt);
  return;
  case 0x3D4:
